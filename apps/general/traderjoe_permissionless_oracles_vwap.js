...
const GRAPH_DEPLOYMENT_ID = 'QmW62QDNJriA73HeyDAwnEaFyDNLZc7o8m5ewSALDRedoM'
...
async function tokenVWAP(token, pairs, metadata) {
  ...
  if (!metadata) {
    ...
    let result = await multiCall(AVALANCHE_ID, contractCallContext)

    metadata = getMetadata(result, PAIRS_INFO)

    let callContextPairs = makeCallContextDecimal(metadata, PAIRS_INFO)
    let resultDecimals = await multiCall(AVALANCHE_ID, callContextPairs)
...
async function LPTokenPrice(token, pairs0, pairs1) {
  ...
  let resultDecimals = await multiCall(AVALANCHE_ID, contractCallContextDecimal)
