...
class ContentVerifyPlugin extends BasePlugin {
  ...
  async onEvent(events, network, contractAddress){
    ...
    console.log(`${network}: num events: ${events.length} blocks: ${JSON.stringify(blocks)}`)
    for(let i in events){
      let {returnValues:{reqId}} = events[i];
      ...
      try {
        ...
        if(!!content){
          let description, verified = false, expectedResult, actualResult;
          [verified, description, expectedResult, actualResult] = await this.verifyContent(content, cid)
  ...
  async verifyContent(content, cid){
    let request, description, verified = false, expectedResult, actualResult;
