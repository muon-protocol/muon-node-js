...
function makeCall(method, params, nSign, mode){
  let callId = newCallId();
  callRedis.publish(GATEWAY_CALL_REQUEST, JSON.stringify({callId, method, params, nSign, mode}))
...
function appCall(app, method, params, nSign, mode){
  let callId = newCallId();
  callRedis.publish(GATEWAY_CALL_REQUEST, JSON.stringify({callId, app, method, params, nSign, mode}))
  let callResult = new CallResult()
  calls[callId] = callResult;
  return callResult.promise
}
